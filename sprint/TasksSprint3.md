...
| T27| Création de la table ``User`` dans ``/Docker/createTable.sql`` contenant les champs ``email``(clé primaire), ``name``, ``password``(qui devra être hashé), ``key``(pour la vérification de compte) et ``active``(pour savoir si le compte à été vérifié).| #1, #2 | | XX jh | To do | Serigne |
