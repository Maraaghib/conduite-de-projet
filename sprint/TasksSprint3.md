...
| T29a | Interface du fichier ``login.php`` dans le répertoire ``src/user`` qui s'occupera de la connexion d'un visiteur | #2 | T27 | 1/2 jh | Done | Guillaume |
| T29b | Implémentaion d'un formulaire sur la page ``login.php`` aves les champs ``mail`` et ``mot de passe`` obligatoire. Un bouton ``Connexion`` en bas du formulaire de finaliser la connexion de l'utilisateur si les champs entrée sont correct et que le visiteur à un compte actif. Si le compte n'éxiste pas alors il est renvoyé sur la page ``register.php`` avec le champ ``mail`` préremplis. Si le compte n'est pas actif il est renvoyer sur une page avec un message lui demandant d'activer son compte, avec le lien d'activation présent dans le mail qu'il à reçu lors de la création du compte, et si un bouton ``Renvoyer le lien d'activation`` est présent afin de revoyer le mail. Lorsqu'un visiteur tente d'accéder à une des pages de l'application, il est renvoyé sur la page ``login.php`` afin qu'il se connecte pour pouvoir accéder au reste de l'application. Lorsque qu'un dévelopeur est authentifié son nom apparait en haut à droite de chaque page. | #2 | T29a | 1 jh | Doing | Guillaume |
| T29c | Ecriture d'un scénario de test pour la connexion d'un visiteur  | #2 |  | 1/2 jh | To do | Guillaume |
| T29d | Execution du test de connexion d'un visiteur et ajout des résultats dans l'historique des tests | #2 | T29c, T29b | 1/2 jh | To do | Guillaume |
| T30a | Interface du fichier ``logout.php`` dans le répertoire ``src/user`` qui s'occupera de la déconnexion d'un dévelopeur | #3 | | 1/2 jh | To do | Guillaume |
...
| T36d | Exécution du test de drag-and-drop des tâches et ajout des résultats dans l'historique des tests | #17 | T36b, T36c | 1/2 jh | To do | Serigne |
