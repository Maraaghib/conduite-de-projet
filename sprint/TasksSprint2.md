...
| T11 | Ajout de la création de la table sprint dans ``createTable.sql`` contenant les champs ``id``,``projectName``, ``dateDébut``, ``dateFin``. Le champs ``id`` est la clé primaire, le champs ``projectName`` est une clé étrangère sur le champs ``projectName`` de la table ``project``. | #13 |  | 1/2 jh | To do |
...
| T14a | Création d'un fichier nommé ``listSprint.php`` dans le répertoire ``src/sprint`` pour le listage des sprints | #19 | T11, T12 | 1/2 jh | To do |
